...
public class when_handling_regular_route_and_bearer_token_is_not_authorized : given.a_request_augmenter
{
    ...
    void Establish()
    {
        ...
        bearer_tokens.Setup(_ => _.Handle(IsAny<HttpRequest>(), IsAny<HttpResponse>(), tenant_id))
            .ReturnsAsync(bearer_token_result);
    }

    async Task Because() => result = await augmenter.Get();

    [Fact]
    void should_return_result_from_bearer_tokens() => result.ShouldEqual(bearer_token_result);
